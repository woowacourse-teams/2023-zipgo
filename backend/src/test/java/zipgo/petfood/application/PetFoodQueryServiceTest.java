...
class PetFoodQueryServiceTest {
    ...
    void 키워드가_다이어트인_식품_목록을_조회한다() {
        ...
        when(petFoodQueryRepository.searchPetFoodByDynamicValues("diet", null)).thenReturn(List.of(다이어트_키워드_식품));
    ...
    void 존재하지_않는_키워드로_조회한다() {
        ...
        // when, then
        assertAll(
                () -> assertThat(petFoodQueryService.getPetFoodByDynamicValue(없는_키워드, null))
                        .hasSize(0)
        );
    }
