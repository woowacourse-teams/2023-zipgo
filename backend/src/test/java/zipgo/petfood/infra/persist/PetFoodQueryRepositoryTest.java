...
class PetFoodQueryRepositoryTest {
    ...
    void 조건에_맞는_식품을_필터링한다() {
        ...
        // when
        List<FilteredPetFoodResponse> petFoodResponses = petFoodQueryRepository.findPagingPetFoods(brandsName, standards,
                primaryIngredientList,
                functionalityList, lastPetFoodId, 20);

        // then
        Assertions.assertAll(
                () -> assertThat(petFoodResponses).hasSize(1),
                () -> assertThat(petFoodResponses).extracting(FilteredPetFoodResponse::foodName)
                        .contains("미국 영양기준 만족 식품"),
                () -> assertThat(petFoodResponses).extracting(petFood -> petFood.brandName())
                        .contains("오리젠")
        );
    }
    ...
    void 같은_식품을_제거하고_limit_개수만큼_반환한다() {
        ...
        // when
        List<FilteredPetFoodResponse> petFoodResponses = petFoodQueryRepository.findPagingPetFoods(brandsName, standards, primaryIngredientList,
                functionalityList, lastPetFoodId, 20);

        // then
        assertThat(petFoodResponses).hasSize(3);
    }
